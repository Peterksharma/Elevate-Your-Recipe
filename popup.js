...
function extractRecipe() {
  ...
  const ingred = document.getElementsByTagName('h2')
  if (ingred) {
  ...
  const ingredientsHeader = Array.from(document.getElementsByTagName('h2')).find(header => 
    ['ingredients', 'recipe'].includes(header.textContent.trim().toLowerCase())
  );

  if (ingredientsHeader) {
    let siblingElement = ingredientsHeader.nextElementSibling;
    while(siblingElement && siblingElement.tagName.toLowerCase() !== 'ul') {
      siblingElement = siblingElement.nextElementSibling;
    }

    if (siblingElement) {
      const ingredientsList = Array.from(siblingElement.getElementsByTagName('li')).map(li => li.textContent.trim());
      console.log('Ingredients:', ingredientsList);
    } else {
      console.error('Ingredients list not found.');
    }
  } else {
    console.error('Ingredients header not found.');
  }
  ...
  if (instructionsList.length > 0) {
    ...
    instructionsList.forEach((instruction, index) => {
      if (instruction.tagName.toLowerCase() !== 'img') {
        ...
        const cleanedText = instructionText.replace(/<img[^>]+>/g, '');
        console.log(`${index + 1}. ${cleanedText}`);
      }
    });
  } else {
